...
def limit_peak_bundle(audios: list[np.ndarray], peak_limit_value: float) -> list[np.ndarray]:
    current_peak = max(peak(audio) for audio in audios)
    if current_peak <= peak_limit_value or current_peak <= 1e-8:
        return [audio.copy() for audio in audios]
    scale = peak_limit_value / current_peak
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build easier TSE datasets from manifests.")
    parser.add_argument("--config", default="configs/dataset_build_easy.yaml")
